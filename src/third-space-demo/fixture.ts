...
const turns: Array<Pick<ThirdSpaceTurn, "reply" | "quickReplies" | "topic">> = [
  ...
  { reply: "Which club or clubs can you currently use? If you have a Group membership, just let me know.", quickReplies: ["Group membership", "Group Plus", "The Wharf", "One club"], topic: "access" },
  { reply: "Which area, station or postcode would make training easy to fit into your day? You can give me more than one.", quickReplies: ["Near Oxford Circus", "Canary Wharf", "Battersea"], topic: "location" },
...
export async function fixtureTurn(messages: DemoMessage[]): Promise<ThirdSpaceTurn> {
  ...
  return { ...turn, readyForMatching: turn.topic === "complete", coverage: { goal: true, experience: index >= 1, membership: index >= 2, access: index >= 3, location: index >= 4, coaching: index >= 5, budget: index >= 6 } };
}

export async function fixtureMatches(messages: DemoMessage[]): Promise<ThirdSpaceMatches> {
  await new Promise((resolve) => setTimeout(resolve, 650));
  const selected = ["kirsty-mclean", "chloe-hubbard", "tom-mans"].map((id) => TRAINERS.find((trainer) => trainer.id === id)!);
  return {
    brief: {
      ...
      membershipType: "group", homeClubIds: [], accessibleClubIds: CLUBS.filter((club) => !["chelsea", "mayfair"].includes(club.id)).map((club) => club.id),
      excludedClubIds: ["chelsea", "mayfair"], locationAnchors: ["Oxford Circus"], budget: "£85–£100", additionalPreferences: [],
    },
