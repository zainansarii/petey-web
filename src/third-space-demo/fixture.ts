...
const turns: Array<Pick<ThirdSpaceTurn, "reply" | "quickReplies" | "topic">> = [
  ...
  { reply: "Which is your home club?", quickReplies: ["Soho", "Canary Wharf", "Battersea"], topic: "access" },
...
export async function fixtureMatches(messages: DemoMessage[]): Promise<ThirdSpaceMatches> {
  ...
  return {
    brief: {
      ...
      membershipType: "group", homeClubIds: ["soho"], accessibleClubIds: CLUBS.filter((club) => !["chelsea", "mayfair"].includes(club.id)).map((club) => club.id),
