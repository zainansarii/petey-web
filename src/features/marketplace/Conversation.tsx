...
export function Conversation({ uid, id, onBack, updated, onChange }: { uid: string; id: string; onBack: () => void; updated: string; onChange: () => void }) {
  ...
  if (!detail) return <section className="mp-panel"><button className="td-text-button" onClick={onBack}><ArrowLeft size={16} /> All enquiries</button><p role="status">{error || "Opening enquiry…"}</p>{error && <button className="td-button" onClick={() => void load().then(setDetail).catch(e => setError(errorMessage(e)))}>Retry</button>}</section>;
  const stopped = Boolean(detail.withdrawnAt || detail.blocked);
  return <section className="mp-conversation"><div className="mp-conversation-heading"><button className="td-text-button" onClick={onBack}><ArrowLeft size={16} /> Back to inbox</button><p>{detail.unlockedAt ? "Unlocked · Free pilot" : "Enquiry preview"}</p></div>
    <div className="mp-conversation-grid"><div className="mp-panel"><h2>{detail.role === "trainer" ? detail.content?.fullName ?? detail.traineeLabel : detail.trainerName}</h2><p className="mp-muted">Received {new Date(detail.createdAt).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}</p>
