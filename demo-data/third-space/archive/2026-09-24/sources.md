...
- Access rules rechecked **25 September 2026**: [membership terms, sections 2.3.4 and 3.1.1](https://www.thirdspace.london/membership-terms-conditions/) define access by membership category, with individual club access dates confirmed on joining. [City](https://www.thirdspace.london/clubs/city/) explicitly offers Single Club (City), Group (excluding Mayfair/Chelsea), and Group Plus (all clubs). [Mayfair](https://www.thirdspace.london/clubs/mayfair/) and [Chelsea](https://www.thirdspace.london/clubs/chelsea/) advertise Group Plus. Do not infer a tier from the home club or invent a separate home-club access matrix.
- For members, learn membership type and home club, then infer training around the home club without a separate location question. Single Club stays at that club; Group/Group Plus considers trainers across every eligible current club, with home and nearby clubs prioritised in ranking. Explicit current-access lists and exclusions override standard entitlements; waiting-list access is not current access.
- Keep inferred geography separate from volunteered preferences: an explicit training area or club restriction overrides the home-club default. Non-members still give a rough area or station. Members who cannot identify their home club can give an area instead. Never request an exact address or postcode.
- Candidate distance uses the home club map point, or the explicit training anchor. All eligible member clubs reach ranking, ordered by distance; the model receives the anchor and distance and favours local trainers of comparable suitability. A stronger evidenced specialist fit can justify a farther club. Non-members explore the nearest three clubs per stated area. These are catalogue options under published rules, not verification of an individual membership account.
